...
`DEMO_MODE=true` seleciona explicitamente a prévia em desenvolvimento, mesmo com o Supabase configurado. O catálogo local reproduz os 144 produtos ativos conferidos em 12 de setembro de 2026, com referências às mídias públicas do Storage e as galerias anteriores preservadas. A prévia não altera o banco de produção. Recursos ativos ficam sempre disponíveis para pedidos, sem controle de quantidade em estoque. Não existe login administrativo alternativo, senha padrão ou bypass de autenticação. Métricas demonstrativas não são armazenadas nem apresentadas como reais.

Na importação de 12 de setembro, os 90 anúncios de referência correspondiam a 89 produtos distintos: Batata numérica de 11 a 20 aparecia duas vezes e Boquinha das Vogais já estava cadastrada. Foram publicados 87 novos produtos e atualizada a imagem da Boquinha. Churrasquinho pedagógico foi cadastrado como rascunho porque sua imagem em alta resolução não veio no segundo arquivo ZIP; ele não integra a prévia pública até receber uma foto e ser ativado.
...
2. Execute as migrações ainda não aplicadas, na ordem de `001_store.sql` até `007_sensory_category.sql`, em `supabase/migrations`. Elas criam catálogo/RLS, descontos, pedidos privados, agendador, cupons, resumo de pedidos e categoria Sensoriais. Aplique antes do deploy. Projetos com tabelas homônimas precisam de migração adaptada.
...
A seção **Em oferta** aparece junto das categorias na navegação e permite buscar pelo nome dentro das promoções. As opções de menor e maior preço usam o valor que será cobrado; **Maior desconto** ordena pela redução percentual. Por exemplo, um produto de R$ 30,00 com preço promocional de R$ 25,00 entra nas ofertas e custa R$ 25,00 por unidade no carrinho e na mensagem do WhatsApp. O preço original deve corresponder ao preço regular real do produto.

As categorias têm links próprios para compartilhar, como `/catalogo/sensoriais`, `/catalogo/alfabetizacao` e `/catalogo/em-oferta`. A seção Sensoriais reúne brinquedos de exploração manual e peças em 3D. O acesso direto, a navegação entre seções e os botões de voltar e avançar mantêm a categoria indicada na URL.
...
As fotos e vídeos de produtos são públicos no bucket `products-media`. Não envie dados pessoais ou documentos particulares para ele. O painel tem botões separados para anexar fotos (JPG, PNG, WebP) e vídeos (MP4, WebM), com progresso e limite de 20 arquivos por produto e 20 MB por arquivo. A galeria permite navegar por setas, teclado e deslize, além de miniaturas na página do produto. Vídeos começam sem som quando visíveis e mostram controles ao tocar. A foto de capa acompanha o carregamento; se o navegador bloquear a reprodução ou a pessoa preferir movimento reduzido, aparece a opção de reproduzir. Para maior compatibilidade, prefira MP4 com vídeo H.264. Os três vídeos HEVC existentes foram convertidos para H.264, preservando os arquivos originais no Storage; novos uploads não passam por conversão automática.
